...
public class SaveWriter {
	...
	public <T> T save(T obj, AnalyzedClass<T> analyzed) {
		try {
			...
			if ((id == null || id == 0l) && analyzed.useAutoGeneration()) {
				...
				saveAutoGeneratedBean(obj, analyzed);
			} else if (id == null) {
				...
				String sql = saveStatement(analyzed, true);
				sql += makeOnDuplicateKeyUpdate(analyzed);
	 ...
	 */
	public <T> void saveAutoGeneratedBean(T obj, AnalyzedClass<T> analyzed) throws IllegalAccessException {
		String sql = saveStatement(analyzed);
		KeyHolder keyHolder = new GeneratedKeyHolder();
		jdbc.update(connection -> {
			try {
				PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
				Object[] args = makeArguments(ps, analyzed, obj);
	...
	public <T> Object[] makeArguments(PreparedStatement ps, AnalyzedClass<T> analyzed, T obj)
			throws IllegalArgumentException, IllegalAccessException {
		return makeArguments(ps, analyzed, obj, false);
	}

	public <T> Object[] makeArguments(PreparedStatement ps, AnalyzedClass<T> analyzed, T obj, boolean forceIdField)
			throws IllegalArgumentException, IllegalAccessException {
		...
		for (Entry<Field, ColumnDefinition<?>> entry : analyzed.getFields().entrySet()) {
			if (analyzed.useAutoGeneration()
					&& entry.getValue() == analyzed.getFields().get(analyzed.getIdField())
					&& !forceIdField)
				continue;
	...
	public String saveStatement(@NonNull AnalyzedClass<?> analyzed, boolean forceIdField) {
		...
		for (Iterator<ColumnDefinition<?>> i = analyzed.getFields().values().iterator(); i.hasNext();) {
			...
			if (analyzed.useAutoGeneration() && col == analyzed.getFields().get(analyzed.getIdField()) && !forceIdField)
				continue;
	...
	public String makeOnDuplicateKeyUpdate(@NonNull AnalyzedClass<?> analyzed) {
		var builder = new StringBuilder(" ON DUPLICATE KEY UPDATE ");
		
		for (Iterator<Entry<Field, ColumnDefinition<?>>> i = analyzed.getFields().entrySet().iterator(); i.hasNext();) {
			Entry<Field, ColumnDefinition<?>> entry = i.next();
			if (entry.getKey() == analyzed.getIdField())
				continue;
		...
		ColumnDefinition<?> idColumn = analyzed.getFields().get(analyzed.getIdField());
